<template>
  <div class="w-full h-screen flex flex-col">
    ...
    <div class="w-full flex flex-wrap mt-auto z-10" v-if="!isRoomMaster">
      <!-- <button
        v-for="answare in game.questions.answare"
        :key="'bukanmaster' + answare"
        @click="submitAnsware(answare)"
      >
        {{ answare }}
      </button> -->

      <!-- <div v-for="answare in game.questions.answare">
        <AnswerCard :color="'red'" :answare="answare"></AnswerCard>
      </div> -->

      <AnswerCard
        v-for="(answare, i) in game.questions.answare"
        :color="colors[i]"
        :answare="answare"
      ></AnswerCard>
    ...
    <div class="w-full flex flex-wrap mt-auto z-10" v-if="isRoomMaster">
      <!-- <span v-for="answare in game.questions.answare" :key="'master' + answare"
        >{{ answare }} |
      </span> -->
  ...
  methods: {
    ...
    nextQuest() {
      ...
      quizilla.doc(this.roomID).update({
        isPlay: true,
        questions: {
          answare: dataNow.answare,
          correct: dataNow.correct,
          image: dataNow.image,
          question: dataNow.question
        }
      })
